...
export default async ({ code }: { code: string }): Promise<PublicInfoMessage> => {
    try {
        ...
        const verifyCode = await models.verificationCode.findOne({
            action: EAction.accountDeactivation,
            code,
        ...
        if (verifyCode) {
            // Find user with id
            await verifyCode.user.set({ status: EStatus.inactive }).save()

            // Expire verification code
            await verifyCode.updateOne({ used: true })

            // Return message
            return new PublicInfoMessage("Your account has been successfully deactivated",
                200)
        }
